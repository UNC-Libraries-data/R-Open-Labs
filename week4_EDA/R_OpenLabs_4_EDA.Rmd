---
title: "R Open Labs: Exploratory Data Analysis (EDA)"
output: 
  html_document:
    toc: true
    toc_float:
...
Huh. There's some extra output that I didn't expect. [A little searching](https://www.google.com/search?q=spec_tbl_df) takes us [here](https://www.tidyverse.org/articles/2018/12/readr-1-3-1/), where we learn that `spec_tbl_df` is a subclass of `tbl_df` that adds a `spec` attribute to keep information about the import process. Whenever we subset using the indexing operator [], R drops that information. So we can exactly recreate the results above like this:
...
R's object-oriented design (which we introduced in [Lesson 2 - Workflows](https://unc-libraries-data.github.io/R-Open-Labs/week2_Workflow/R_OpenLabs_2_Workflow.html#3:_objects_and_classes)) allows this kind of behavior. We usually don't need to concern ourselves with this, but it's good to know enough about it to search for answers when you run into problems. 
