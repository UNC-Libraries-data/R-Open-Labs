...
library(stargazer) #nice reports for linear models
```
...
A model aims to create a simple approximation of actual relationships in a dataset.  This approximation makes it easier to interpret and generalize relationships to new data.  Ideally a model represents the "signal" in the data while ignoring unimportant or ungeneralizable "noise.""
...
For these plots, we'll use `moddf` a fake dataframe I've created.  

We'll look at the code to generate `moddf` at the end of this section.

```{r}
moddf <- read.csv(url("https://github.com/UNC-Libraries-data/R-Open-Labs/raw/master/week8_Models/moddf.csv"))
str(moddf)
```

Since these are both continusous numbers, let's plot them with a scatterplot.

```{r fig.width=4, fig.height=4}
ggplot(data=moddf,aes(x=x,y=y))+geom_point()+theme_bw()
```

One of the simplest ways to model data like this is with a line capturing the trend.  We can fit a line with an interpretable formula: 

$$y= intercept + slope * x$$

We can plot candidate line(s) with `geom_abline`:
...
OLS can accomodate multiple predictors.  Let's experiment with the built-in `mtcars` dataset.
...
**Note**: `caret` is just a wrapper for fitting models - it does not include functions to fit many of these models.  When fitting a model you may see the following message:
```
...
#XGBoost (a refinement of Random Forests)
...
Let's compare how our models perform.

The `resamples` function considers the models against datasets simulated by sampling from the training set with replacement.  This concept is related to "bootstrapping".
...
These track how well the model fits the data in different ways.  Without getting into the details of how they're calculated, we'll simply use the rules of thumb that:

* **Lower** Errors (MAE and RMSE) are better
* **Higher** R-squared values are better

Note: `caret` provides different metrics (Kappa and accuracy) for classification (categorical outcome) tasks.
...
The random forest and xgbTree seem to be doing well here, but it's not clear
...
Based on these results xgbTree seems to perform best for predicting Sacramento house prices in the testing data.
