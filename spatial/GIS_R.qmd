...
```         
install.packages(c('sf','nngeo','tidycensus','tigris','arcgislayers','tmap','ggmap','ggspatial','basemap'))
```
...
```{r}
all(housing_raw$X==housing$LON)
all(housing_raw$Y==housing$LAT)
```
...
There are a number of ways to get basemaps in R, but for today we'll use the `basemaps` package.
