...
The `load_variables` function provides a list of the available tables and variables for a given year and census survey, for example 5-year ACS data below:
...
ggplot()+
  ...
  geom_sf(data = tri_counties, color = "grey80", fill=NA, lwd = 0.8)+
  scale_fill_gradient2(low="darkred",high="darkblue", mid="white", labels=scales::percent)+
  theme_minimal()
```
