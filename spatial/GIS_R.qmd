...
basemaps::basemap_ggplot(ext = st_bbox(housing), map_service = "carto")+
  ...
  coord_sf(crs = 3857)+ #ensures all layers are projected into the correct CRS
  ggmap::theme_nothing()
```
...
We can add a state boundary and some space using `tigris` and `st_buffer`.  Note that we need to use `st_transform` to project the data to match the basemap before running a buffer.
...
Let's add state maintained roads to the map!  We'll use the ESRI data service from NC One Map.
["NC DOT State Maintained Roads"](https://www.nconemap.gov/maps/157dbc4ef33f4db4aa1ecc1a3182a375/explore) is a *Map Service* that contains multiple layers.  

To load data, we'll need to specify a particular layer. On OneMap, you can click "I want to use this" > "View Data Source" to get to the MapServer directory page:
[https://gis11.services.ncdot.gov/arcgis/rest/services/NCDOT_StateMaintainedRoadsQtr/MapServer](https://gis11.services.ncdot.gov/arcgis/rest/services/NCDOT_StateMaintainedRoadsQtr/MapServer)

This gives us most of what we'll need to load the layer, but we still need to pick a layer under the "Layers" list.  For today, let's look at the simplest layer - Interstates (layer 2):
[https://gis11.services.ncdot.gov/arcgis/rest/services/NCDOT_StateMaintainedRoadsQtr/MapServer/2](https://gis11.services.ncdot.gov/arcgis/rest/services/NCDOT_StateMaintainedRoadsQtr/MapServer/2)
...
Here we can add some more polish to the plot:
- add colors for Urban vs Rural and label appropriately, update labels.
- switch to the "light_no_labels" basemap from carto (see `get_maptypes(map_service = "carto")` for a full list of options).
- add a scale and north arrow for context
- various color and line tweaks for aesthetics
...
basemaps::basemap_ggplot(ext = st_bbox(st_buffer(nc_boundary, 
  ...
  ggspatial::annotation_scale(pad_x = unit(1.5, "cm"))+
...
This fails because we haven't made sure both our objects are in the same CRS. Let's check.
...
st_crs(us_csas)
```
Let's go with NAD83 for now, since that's the default for census and tigris data.
...
```{r}
ggplot()+
  geom_sf(data = nc_csas, aes(fill = CSAFP))+
  geom_sf(data = nc_boundary, fill = NA, linetype = "dashed")+
  theme_minimal()
```

```{r}
csa_housing <- st_join(nc_csas, housing_nad) |> 
  st_drop_geometry() |> 
  group_by(CSAFP) |> 
  summarize(ct = n(),
